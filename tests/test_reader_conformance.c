/*
 * Fixed version of libyaml's test-reader.c
 *
 * The original libyaml test has a memory leak in check_utf8_sequences():
 * when the '!' terminator is hit, it breaks out of the inner loop without
 * calling yaml_parser_delete() on the last parser instance. This causes
 * ASAN to report leaks (42 leaked parsers, ~3.6MB).
 *
 * This fixed version adds the missing yaml_parser_delete() call after
 * the break, making the test ASAN-clean.
 */
